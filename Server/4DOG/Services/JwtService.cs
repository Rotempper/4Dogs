...
namespace _4DOG.Services
{
    public class JwtService
    {
        ...
        private TimeSpan TokenNumMinutesToExtend = new TimeSpan(1, 1, 0); // זמן טוקן
        ...
        public string GenerateToken(string tokenPrimaryValue, string role)
        {
            ...
            else if (role == "DogOwnerRole") 
            {
                tokenDescriptor = new SecurityTokenDescriptor
                {
                    ...
                    {
                        new Claim(TokenPrimaryKey, tokenPrimaryValue),
                        new Claim(ClaimTypes.Role,"DogOwnerRole")
                    }),
                    Expires = expired,
                    SigningCredentials = new SigningCredentials(JwtSymmetricSecurityIssuerSigningKey, SecurityAlgorithms.HmacSha512),
                    EncryptingCredentials = new EncryptingCredentials(JwtSymmetricSecurityTokenDecryptionKey, SecurityAlgorithms.Aes128KW, SecurityAlgorithms.Aes128CbcHmacSha256)
            ...
            else if (role == "BarberShopRole") 
            {
                tokenDescriptor = new SecurityTokenDescriptor
                {
                    ...
                    {
                        new Claim(TokenPrimaryKey, tokenPrimaryValue),
                        new Claim(ClaimTypes.Role,"BarberShopRole")
                    }),
                    Expires = expired,
                    SigningCredentials = new SigningCredentials(JwtSymmetricSecurityIssuerSigningKey, SecurityAlgorithms.HmacSha512),
                    EncryptingCredentials = new EncryptingCredentials(JwtSymmetricSecurityTokenDecryptionKey, SecurityAlgorithms.Aes128KW, SecurityAlgorithms.Aes128CbcHmacSha256)
            ...
            else if (role == "PensionRole") 
            {
                tokenDescriptor = new SecurityTokenDescriptor
                {
                    ...
                    {
                        new Claim(TokenPrimaryKey, tokenPrimaryValue),
                        new Claim(ClaimTypes.Role,"PensionRole")
                    }),
                    Expires = expired,
                    SigningCredentials = new SigningCredentials(JwtSymmetricSecurityIssuerSigningKey, SecurityAlgorithms.HmacSha512),
                    EncryptingCredentials = new EncryptingCredentials(JwtSymmetricSecurityTokenDecryptionKey, SecurityAlgorithms.Aes128KW, SecurityAlgorithms.Aes128CbcHmacSha256)
            ...
            else
            {
                tokenDescriptor = new SecurityTokenDescriptor
                {
                    ...
                    {
                        new Claim(TokenPrimaryKey, tokenPrimaryValue),
                        new Claim(ClaimTypes.Role,"TrainingRole")
                    }),
                    Expires = expired,
                    SigningCredentials = new SigningCredentials(JwtSymmetricSecurityIssuerSigningKey, SecurityAlgorithms.HmacSha512),
                    EncryptingCredentials = new EncryptingCredentials(JwtSymmetricSecurityTokenDecryptionKey, SecurityAlgorithms.Aes128KW, SecurityAlgorithms.Aes128CbcHmacSha256)
